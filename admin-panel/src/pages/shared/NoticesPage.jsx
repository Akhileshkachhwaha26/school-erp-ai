...
const CAN_CREATE = ["admin", "teacher"];
...
export default function NoticesPage() {
  ...
  const loadNotices = () => {
    api.get("/notices").then(({ data }) => setNotices(data.notices || [])).catch(() => toast.error("Could not load notices")).finally(() => setLoading(false));
  };
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
            <Megaphone className="h-6 w-6 text-orange-500" /> Notices & Circulars
          </h1>
      ...
      {loading ? <p className="text-sm text-slate-400">Loading...</p>
        : notices.length === 0 ? (
          ...
          <div key={n._id} className="mb-3 rounded-xl border border-slate-200 bg-white p-5">
            ...
            <p className="mt-2 text-xs text-slate-400">Posted by {n.createdBy?.name || "Admin"}</p>
          </div>
        ))
      }
    </div>
